...
@section('content')
    <div class="page-header">
        ...
        <div class="page-btn">
            <button class="btn btn-primary mb-2 btnCreateModal" data-toggle="modal" data-target="#createModal123">Tambah Permission</button>
        </div>
    </div>
    <div class="card">
        <div class="card-body">
...
@endsection
 @push('scripts')
    <script>
        ...
        //     $.get("{{ route('permissions.list') }}", function(data) {
        //         let rows = '';
        //         data.forEach(item => {
        //             rows += `
        //         <tr>
        ...
        //             <td>
        //                 <button class="btn btn-sm btn-info editBtn" data-id="${item.id}" data-name="${item.name}" data-guard="${item.guard_name}">Edit</button>
    ...
    $.get("{{ route('permissions.list') }}?page=" + page, function(data) {
        let rows = '';
                ...
                $.ajax({
                    ...
                    error: err => {
                        if (err.status === 422) {
                            ...
                            if (errors.guard_name) $('.error-edit-guard').text(errors
                                .guard_name[0]);
                        }
                    }
                });
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: res => {
                                loadPermissions();
                                Swal.fire('Dihapus!', 'Permission berhasil dihapus',
                                    'success');
                            }
                        });
                    }
