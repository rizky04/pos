        ...
        <div class="card-body">
            ...
            <div class="row mt-3">
                <div class="col-lg-4">
                    ...
                    <select id="status_bayar" name="status_bayar" class="form-select" required>
...
@push('scripts')
    ...
    <script>
                ...
                ajax: {
                    ...
                    processResults: data => ({
                        results: data.map(item => ({
                            id: item.id_barang,
                            text: `${item.id_barang} - ${item.kode_barang} - ${item.nama_barang} - ${item.jenis} - ${item.keterangan} - (Stok: ${item.stok_barang ?? 0}) - Harga: Rp ${item.harga_jual.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")} `,
                            nama: item.nama_barang,
                            kode: item.kode_barang,
                            harga: item.harga_jual,
                            harga_kulak: item.harga_kulak,
        ...
        `;
            ...
            // ==================== AUTO FILL AMOUNT PAID & KEMBALIAN ====================
...
//         // hanya auto-update jika user tidak sedang mengetik manual
//         amountPaidInput.val(grandTotal);
//     }

//     updateChangeAmount(); // selalu update kembalian juga
// }
...
function updateChangeAmount() {
    ...
    $('#change_amount').val(formatRupiahInput((change > 0 ? change : 0).toString()));
            ...
            function calculateGrandTotal() {
                ...
                //  syncAmountPaid(); // ⬅️ tambahkan ini
                syncAmountPaid(); // otomatis isi dan hitung kembalian
            }

            // update kembalian setiap kali user ubah amount_paid
// $('#amount_paid').on('input', function() {
//     updateChangeAmount();
// });
// setiap user ubah input pembayaran, update format & kembalian
$('#amount_paid').on('input', function() {
    const raw = $(this).val();
    ...
    updateChangeAmount();
            ...
            $('#btnSimpan').on('click', function() {
                ...
                $.ajax({
                    ...
                    error: err => {
                        Swal.fire('Gagal', 'Terjadi kesalahan saat menyimpan transaksi. ' + (err
            ...
            $.get("{{ route('select2.clients') }}", {
                    q: 'walkin'
                })
                .done(function(data) {
                    const walkin = data.find(c =>
                        (c.nama_client ?? c.text ?? '').toLowerCase() === 'walkin'
                    );

                    if (walkin) {
                        const option = new Option(
                            walkin.nama_client ?? walkin.text,
                            walkin.id_client ?? walkin.id,
                            true,
                            true
                        );
                        $('#id_client').append(option).trigger('change');
                    } else {
                        // fallback kalau ga ada di DB, buat manual
                        const option = new Option('walkin', 0, true, true);
                        $('#id_client').append(option).trigger('change');
                    }
                })
                .fail(function() {
