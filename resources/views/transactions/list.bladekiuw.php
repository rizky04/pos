...
<script>
// ========================================
// TRANSACTION LIST - COMPLETE SCRIPT
// Pastikan tidak ada duplicate event handlers
// ========================================
...
$(function() {
    ...
    // Event ketika edit modal ditutup
        ...
        tbody.append(`
            <tr>
                ...
                <td class="text-end">
                    ...
                    <button class="action-btn btn-edit" data-id="${t.id}">
...
function setupEditEventHandlers() {
    ...
    // Input qty/price berubah
    $('#editItemsTable').on('input change', '.qty, .price', function () {
        let tr = $(this).closest('tr');
        calculateRow(tr);
    });

    // Product select berubah
    $('#editItemsTable').on('change', '.product-select', function () {
    ...
    // Button remove item
    $('#editItemsTable').on('click', '.btn-remove', function () {
        let tr = $(this).closest('tr');
        console.log('Removing row:', tr.data('id'));
        tr.remove();
        calculateEditSummary();
        console.log('Remaining rows:', $('#editItemsTable tbody tr').length);
    });

    // Discount/PPN/Payment berubah
    $('#discountValue, #discountType, #ppnValue, #payAmount').on('input change', function() {
        calculateEditSummary();
    });
...
function openEditModal(transactionId) {
    console.log('Opening edit modal:', transactionId);
    resetEditModal();
    editModal.show();
...
function addEditItemRow(item = null) {
    ...
    if (item) {
        ...
        console.log('Item added:', {rowId, product_id: item.product_id, qty: item.qty, price: item.price});
    }

    calculateRow(tr);
    console.log('Total rows:', $('#editItemsTable tbody tr').length);
}
...
function calculateEditSummary() {
    ...
    console.log('Summary:', {itemCount, subTotal});
...
function buildEditItemsPayload() {
    ...
    $('#editItemsTable tbody tr').each(function () {
        ...
        if (productId && qty > 0 && price > 0) {
            items.push({product_id: productId, qty: qty, price: price});
        }
    });
...
function submitEditTransaction(items) {
    ...
    $.ajax({
        ...
        error: function(err) {
            console.error('Error:', err);
            alert('Gagal update: ' + (err.responseJSON?.message || 'Unknown error'));
        }
    });
}
