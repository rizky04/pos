...
function addEditItemRow(data = {}) {
    ...
    let row = $(`
        <tr>
            ...
            <td>
                <input type="number" class="form-control form-control-sm edit-price" value="0" readonly>
            </td>
            <td class="edit-subtotal">Rp 0</td>
            <td>
                <button class="btn btn-sm text-danger btn-remove">X</button>
    ...
    // ===============================
    // SET PRODUCT & PRICE (INI KUNCI)
    // ===============================
    if (data.product_id) {
        row.find(".edit-product").val(data.product_id);

        let product = products.find(p => p.id == data.product_id);
        if (product) {
            row.find(".edit-price").val(data.price ?? product.harga_jual);
        }
    }
